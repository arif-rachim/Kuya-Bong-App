...
// input: { appointmentId, therapistId, clinicId, date, start, end,
...
module.exports = async (input, ctx) => {
  ...
  await ctx.fetch(`${base}/data/audit_log`, {
    ...
    body: JSON.stringify({ actor_user_id: i.actorUserId || null, actor_name: i.actorName || 'Admin', action: 'Reschedule appointment', detail: `${apt.for_member_name || ''} · ${apt.date} ${apt.start} -> ${i.date} ${i.start}`, at: new Date().toISOString() }),
  })
