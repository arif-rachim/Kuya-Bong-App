...
// input: { appointmentId, action, patientPackageId?, reasonId?, note?,
...
module.exports = async (input, ctx) => {
  ...
  if (i.action === 'complete' && i.patientPackageId) {
    ...
    await ctx.fetch(`${base}/data/package_usage`, { method: 'POST', headers: jh, body: JSON.stringify({ patient_package_id: i.patientPackageId, appointment_id: apt.id, member_name: apt.for_member_name || '', date: today(), recorded_by: i.actorName || 'Admin' }) })
  }
  ...
  await ctx.fetch(`${base}/data/audit_log`, { method: 'POST', headers: jh, body: JSON.stringify({ actor_user_id: i.actorUserId || null, actor_name: i.actorName || 'Admin', action: labels[i.action], detail: `${apt.for_member_name || ''} · ${apt.date} ${apt.start}`, at: new Date().toISOString() }) })
