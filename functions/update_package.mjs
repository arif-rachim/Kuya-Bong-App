...
// input: { packageId, remaining, name, __callerToken }
...
module.exports = async (input, ctx) => {
  ...
  await ctx.fetch(`${base}/data/audit_log`, { method: 'POST', headers: { 'x-api-key': key, 'content-type': 'application/json' }, body: JSON.stringify({ actor_user_id: caller.id, actor_name: caller.name || 'Admin', action: 'Edit package remaining', detail: `${i.name}: ${cur.data.remaining} -> ${remaining}`, at: new Date().toISOString() }) })
